...
//  FMDBTut
//
//  Created by Manoj pratap singh rana on 05/04/17.
//
...
/*
 ...
 //2) Open database
        database.open()
 //3) Create Table
            query = let createMoviesTableQuery = "create table movies (movieID integer primary key autoincrement not null, title text not null, category text not null, year integer not null, movieURL text, coverURL text not null, watched bool not null default 0, likes integer not null)"
            database.executeStatements(query) (query is sql query)
